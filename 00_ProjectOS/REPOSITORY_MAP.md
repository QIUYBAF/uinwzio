...
3. 不继续往这些目录加入新项目资产；
4. 普通项目任务不得顺手修改；每日治理任务可在完成依赖与价值审计后主动删除或收敛；
5. 有明确历史、展示或复现价值的内容才迁入 `90_Legacy/legacy_site/`；无价值且无依赖的内容直接删除。
...
这不是最终物理迁移结果，而是**当前唯一逻辑结构**。后续治理应一次性完成依赖审计、必要迁移、引用修复与无价值内容删除，不让 Frozen Legacy 成为永久垃圾场。
